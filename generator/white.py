...
class WhitePlate(LicensePlateGenerator):

    def __init__(self):
        super(WhitePlate, self).__init__()
        self.length = 5
    ...
    def generate(self):
        ...
        for index, char in enumerate(list(plate)):
            char_image = Image.open(os.path.join(
                'generator', 'images', 'white', f'{char}.png')).convert('RGBA')
        ...
        canvas_image = Image.new("RGBA", image.size, "WHITE")
...
if __name__ == '__main__':
    ...
    generator = WhitePlate()
    image, plate = generator.generate()
    # image.save(f'output/white_{plate}.png')
